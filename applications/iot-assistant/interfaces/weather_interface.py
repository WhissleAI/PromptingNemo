...
def weather_agent(prompt):
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system", 
                "content": "In the query, you extract a location from the user's request. Output only location name string as output, not any other information."
            },
            {
                "role": "user", 
                "content": prompt
    ...
    location_response = requests.post(NVAPI_URL, headers=headers, json=data)
    location_response = location_response.json()['choices'][0]['message']['content'].strip()
