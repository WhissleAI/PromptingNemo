...
class PunctuationCapitalizationLexicalAudioModel(PunctuationCapitalizationModel):
    ...
    def forward(
        ...
        ## change: replace audio with lexical,
        '''
        missing lexical_hidden_state_length mapping
        '''
        fused = self.fusion(
            audio_hidden_states,
            attention_mask,
            lexical_hidden_states,
            self.make_pad_mask(lexical_hidden_states.size(1), lexical_hidden_states_length),
        )
