...
def make_readme(meta_info, example_samples):
    readme = """# Meta Speech Recognition Hindi Dataset (Set 1)
...
def main():
    ...
    parser.add_argument("--input_dir", type=str, default="/external1/datasets/hf/hindi")
    parser.add_argument("--repo_id", type=str, default="WhissleAI/Meta_STT_HI_Set1")
    ...
    sources = ["ai4bharat", "commonvoice", "shrutilipi"]
    dataset_dict = {}
    meta_info = {}
    example_samples = {}
    ...
    for split in splits:
        ...
        for source in sources:
            json_file = os.path.join(args.input_dir, split, f"{split}_{source}.json")
            if not os.path.exists(json_file):
                print(f"File not found: {json_file}")
                continue
                
            print(f"Reading {split} split from {source}...")
            samples = read_json_file(json_file)
            if samples:
                all_samples.extend(samples)
            else:
                print(f"No samples found for split '{split}' from source '{source}'.")
