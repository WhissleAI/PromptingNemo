...
# log_file: logs representing output
#
...
def create_ctc_plot(index, probs, alignments,labels):
    ...
    for i in index:
        ...
        y_probs[i] = str(probs[k])
        labels_new[i] = str(labels[k])
        k = k + 1

    return length, y_probs, labels_new




def get_score(probs, hyp):
    ...
    for k in range(0,len(hyp)):
        ...
        index = [i for i, e in enumerate(hyp[k].alignments) if list(e).index(max(list(e))) != 1024]
        labels = [list(e).index(max(list(e))) for i, e in enumerate(hyp[k].alignments) if list(e).index(max(list(e))) != 1024]
#        print("\n---Time steps---\n", len(hyp[k].alignments),"\n")

        probi = [probs[k][i] for i in index]
        probi = list(probi)
#        print("\n---Probs---\n", probi, "\n")
        ...
        for prob in probi:
            prob = list(prob)
            hyp_probs.append(np.exp(max(prob)))
            hyp_prob = hyp_prob + max(prob)
            try:
                score = np.exp(hyp_prob/len(probi))
            except:
                score = 0.0
            scores.append(score)
#        print("\n---hyp probs---\n", hyp_probs, "\n")

        length, y_probs, labels_new = create_ctc_plot(index, hyp_probs, hyp[k].alignments,labels)
        
        lengths.append(length)
        y_probss.append(y_probs)
        labels_news.append(labels_new)

    return scores, lengths, y_probss, labels_news
...
def main(cfg: TranscriptionConfig):
    ...
    with autocast():
        with torch.no_grad():
            ...
            probs = asr_model.transcribe(filepaths, batch_size=cfg.batch_size,logprobs=True)
            hyp = asr_model.transcribe(filepaths, batch_size=cfg.batch_size,return_hypotheses=True)
            #print(hyp)
            scores, lengths, y_probss, labels_news = get_score(probs, hyp)
            #print(scores)
    ...
    with open(cfg.output_filename, 'w', encoding='utf-8') as f:
        if cfg.audio_dir is not None:
            for idx, text in enumerate(transcriptions):
                item = {'audio_filepath': filepaths[idx], 'pred_text': text, 'Score': "score"}
            ...
            with open(cfg.dataset_manifest, 'r') as fr:
                for idx, line in enumerate(fr):
                    ...
                    item['length'] = lengths[idx]
                    item['probs'] = y_probss[idx]
                    item['labels'] = labels_news[idx] 
