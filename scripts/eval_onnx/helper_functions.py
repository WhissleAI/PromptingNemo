...
import logging
import math
import random  # If you're using the random module in your code
...
openai.api_key = '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'
...
class TextToSpeech:
    ...
    def list_available_models(self):
        return self.tts_model.list_models()

    def tts(self, text, speaker_wav, language):
        wav = self.tts_model.tts(text=text, speaker_wav=speaker_wav, language=language)
        return wav

    def tts_to_file_cloning(self, text, speaker_wav_folder, language, file_path):
        # Get a list of all audio files in the specified folder
        audio_files = [file for file in os.listdir(speaker_wav_folder) if file.endswith(".wav")]

        if not audio_files:
            raise ValueError("No WAV files found in the specified folder.")

        # Select a random audio file from the list
        selected_speaker_wav = os.path.join(speaker_wav_folder, random.choice(audio_files))

        # Save the generated audio to the specified file path
        self.tts_model.tts_to_file(text=text, speaker_wav=selected_speaker_wav, language=language, file_path=file_path)

    def tts_to_file_gtts(self, text, language, file_path):
...
def transcribe_audio_onnx():
    ...
    file_path = os.path.join('/home/ubuntu/advanced-speech-LLM-demo/asr-nl_onnx/demo_audio', audio_file)
...
print("---Initiate ONNX sessions for ASR-NL models----")
ort_session_en_ner, model_tokenizer_en, filterbank_featurizer = create_ort_session(model_name="EN_ner_emotion_commonvoice")
ort_session_en_pos, model_tokenizer_en, filterbank_featurizer = create_ort_session(model_name="EN_pos_emotion_commonvoice")
ort_session_euro_ner, model_tokenizer_euro, filterbank_featurizer = create_ort_session(model_name="EURO_ner_emotion_commonvoice")
ort_session_en_noise, model_tokenizer_noise, filterbank_featurizer = create_ort_session(model_name="EN_noise_ner_commonvoice_50hrs")
